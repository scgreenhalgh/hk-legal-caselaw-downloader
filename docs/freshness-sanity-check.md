...
|---|---|---|
| UKPC has 5 permanent HKLII gaps (4× 404, 1 empty content). `local=237 vs live=242` never converges. Specific IDs: 1987/3, 1988/2, 1993/3, 1995/4 (404), 1997/4 (empty content). | UKPC/en bucket is perpetually STALE → `scrape_ukpc` runs every weekly+ update. Costs ~5 wire calls per run. | Trivial cost; not worth engineering around. |
| **HKLII duplicate listing** — `gethoptfiles?dbcat=other&abbr=bahkg&lang=en` returns 218 entries but one path (`/en/legis/bahkg/1991/4/`) is duplicated. We have all 217 unique records. Observed 2026-07-08. | bahkg/en perpetually STALE with `delta=+1`. `scrape_hopt` re-enumerates every weekly+ update (~1 page of wire). | Trivial cost. If HKLII de-dups the listing it'll self-heal. |
| **HKLII list-vs-fetch gap** — `hkts/tc/1961/2` is listed by `gethoptfiles` but `gettreaty?lang=tc&abbr=hkts&year=1961&num=2` returns HTTP 404. Same shape as UKPC's permanent 4× 404. Observed 2026-07-08. | hkts/tc perpetually STALE with `delta=+1`. Row sits at status='failed' in `hopt_documents`. | Trivial cost. Won't self-heal unless HKLII publishes the treaty content. |
| **`hkdc/2019/128` has no TC translation** — HKLII returns empty content for the getjudgment TC call even though `has_translation=True` in the EN JSON. Documented in `docs/backup-coverage-2026-07-06.md`. Observed 2026-07-06, confirmed still permanent 2026-07-08. | hkdc/tc perpetually STALE with `delta=+1`. `backfill_case_translations` finds the row, calls getjudgment, gets empty content, logs failure — no local sidecar written. | Accept as permanent HKLII-side gap. |
...
| **pcpdaab TC 2019/8 — phantom HKLII entry**. HKLII's TC listing includes `pcpdaab/2019/8` (title "AAB 8-2019", neutral "[2019] HKPCPDAAB 8"), but every avenue for actual case content fails: PCPD's `decisions_detail.html` doesn't list AAB 8-2019 in either language; every filename variant on `pcpd.org.hk/{english,tc_chi}/enforcement/decisions/files/` returns 404; HKLII's own `/api/getother?year=2019&num=8&lang=tc` returns metadata whose internal `path` field says `/2018/8/` (contradicting the listing); `casenotes_2.php?id=2019A08` returns content for **Appeal No. 27 of 2019** (unrelated). HKLII EN listing does not have 2019/8 at all. Observed 2026-07-09. | pcpdaab/tc perpetually STALE at 367/368 (`local < live` by 1). Row sits at status='failed' in `hopt_documents`. | Accept as permanent HKLII-side data glitch — likely an accidental duplicate row for `pcpdaab/2018/8` that got mislabeled with year 2019 in HKLII's TC index. Won't self-heal unless HKLII corrects the listing OR PCPD publishes the case. |
