...
`additionalProperties: false` so that new wire fields surface as
validation failures rather than pass silently. Schemas are validated
against real samples on every commit via
...
| Timestamps | ISO 8601. Metadata dates use `+08:00` (Hong Kong Time). Events log entries use UTC (`+00:00`) with microsecond precision. |
...
output/
  ...
  d3/{family}/{year}/{num}/
    {family}_{year}_{num}_{lang}.json                  (hklrccp, hklrcr, pcpdc)
    pcpdaab_{year}_{num}_{lang}.{json,pdf,txt}         (pcpdaab: JSON + PDF pair)
  legis/{abbr}/{cap}/
...
```json
{
  ...
  "parallel_citations": ["(2020) 23 HKCFAR 100"],
...
| File | Schema | Purpose |
|---|---|---|
| `{stem}.noteup.json` | [`noteup.schema.json`](schemas/noteup.schema.json) | Array of judgments that cite this one — from `/api/getnoteups`. Empty array = never cited. |
| `{stem}.appeal_history.json` | [`appeal-history.schema.json`](schemas/appeal-history.schema.json) | Chain of dockets across the appeal — from `/api/getappealhistory`. Typically 1–4 entries. |
...
(Basic Law consultation papers), `bahkg` (Basic Law HK Gazette), `hktmc`
(marine codes), `hktml` (multilateral treaties), `hkts` (HK Treaty Series).
...
**Files:** `pcpdaab_{year}_{num}_{lang}.{json,pdf}`, plus `.txt` sidecars
for some TC rows.
...
Language quirk: `pcpd.chinese_only:true` means HKLII lists the row under
`en` but the actual PDF is Chinese-only. **Trust this flag over the `_en`
filename** when determining the language of the underlying decision.
...
`oneOf` discriminated on the `kind` field. Nine kinds observed:
...
| `request_failed` | 670 | `proxy_url, url, elapsed_ms, error_class, error_msg` |
...
**Location:** `output/failure_samples/challenge_{court}_{year}_{num}.{html,headers.json}`.
...
The script samples a small number of files per artifact type, validates
them against the corresponding schema, and exits non-zero on any failure.
It also spot-checks the first 200 lines of `events.jsonl`. Extend
`SAMPLES_PER_TYPE` (currently 5) for a deeper sweep.
